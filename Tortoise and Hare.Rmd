...
qt(p=0.05/2,df=18)
```
At the level of 5% significance, the rejection region is {t>2.10} $\cup$ {t<-2.10}. So the t score of -0.555 is not in the rejection region. We can conclude that we do not reject the null hypothesis of $\overline{x}$ = $\overline{y}$. 
...
$$ 
\begin{align}
E\left[U_{hare}\right] &= E[\sum_{i}^{10} \sum_{j}^{10}I ] \\
